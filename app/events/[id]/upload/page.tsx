...
setFiles(prev=>[...prev,...arr.map(f=>({file:f,preview:URL.createObjectURL(f),caption:''}))])
}
...
const updateCaption=(i,val)=>setFiles(prev=>prev.map((f,idx)=>idx===i?{...f,caption:val}:f))
...
style={{border:'2px dashed '+(dragOver?'#ff6b00':'#e5e7eb'),borderRadius:'10px',padding:'40px',textAlign:'center',marginBottom:'24px',background:dragOver?'#fff8f3':'#fafafa',cursor:'pointer',transition:'all 0.2s'}}
...
<input id='fileInput' type='file' multiple accept='image/*' style={{display:'none'}} onChange={(e)=>handleFiles(e.target.files)}/>
</div>
{uploading&&(
...
{files.length>0&&(
<div>
<div style={{fontSize:'13px',fontWeight:600,color:'#374151',marginBottom:'12px'}}>{files.length} images ready to upload</div>
...
<input value={f.caption} onChange={(e)=>updateCaption(i,e.target.value)} placeholder='Caption...' style={{width:'100%',border:'1px solid #e5e7eb',borderRadius:'4px',padding:'4px 6px',fontSize:'11px',marginTop:'4px',outline:'none'}}/>
</div>
))}
</div>
</div>
